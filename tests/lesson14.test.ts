...
import {LessonApiClient1} from "../src/controllers/lesson-api-client1"
import {LessonApiClient2} from "../src/controllers/lesson-api-client2";
...
// Lesson 14
    ...
    test('1 POST create n users', async ({request}) => {
        const apiClient = await LessonApiClient1.getInstance(request)
    ...
    test('2 DELETE n users', async ({request}) => {
        const apiClient = await LessonApiClient2.getInstance(request)
